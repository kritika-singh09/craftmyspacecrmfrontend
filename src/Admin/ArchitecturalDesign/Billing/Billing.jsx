...
import { FiDollarSign, FiPieChart, FiFileText, FiPlus, FiArrowUpRight, FiCreditCard, FiCheckCircle, FiClock, FiSettings, FiShield, FiZap, FiInfo, FiX, FiCheck } from 'react-icons/fi';
...
const Billing = () => {
    ...
    const [newInvoice, setNewInvoice] = useState({ project: '', amount: '', type: 'Phase 1: Concept' });
    ...
    const handleCreateInvoice = (e) => {
        e.preventDefault();
        const invoice = {
            id: `INV-2024-0${invoices.length + 10}`,
        ...
        setNewInvoice({ project: '', amount: '', type: 'Phase 1: Concept' });
    };
    ...
    return (
        <div className="space-y-10 pb-12 animate-in fade-in duration-700">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                <div>
                    <h1 className="text-4xl font-black tracking-tight" style={{ color: theme.textPrimary }}>FEES & <span style={{ color: theme.secondary }}>BILLING</span></h1>
                ...
                <div className="flex gap-4">
                    <div className="flex p-1 rounded-2xl border" style={{ backgroundColor: `${theme.primary}05`, borderColor: theme.cardBorder }}>
            ...
            {activeTab === 'invoices' ? (
                <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {[
                            ...
                            { label: 'Received', val: '₹1.8 Cr', icon: <FiCheckCircle />, accent: theme.success },
                            { label: 'Pending', val: '₹2.4 Cr', icon: <FiClock />, accent: theme.warning },
                        ].map((stat, i) => (
                            <div key={i} className="p-8 group rounded-[2.5rem] shadow-premium border transition-all hover:scale-[1.02]" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                    ...
                    {/* Invoice Table */}
                    <div className="rounded-[3rem] shadow-premium border overflow-hidden" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                        <div className="p-8 border-b flex justify-between items-center" style={{ borderColor: theme.cardBorder, backgroundColor: `${theme.primary}05` }}>
                            <h4 className="text-lg font-black uppercase tracking-tight" style={{ color: theme.textPrimary }}>Recent Invoices</h4>
                            <FiArrowUpRight className="cursor-pointer hover:scale-110 transition-transform" style={{ color: theme.primary }} />
                        </div>
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y" style={{ borderColor: theme.cardBorder }}>
                                {invoices.map((inv) => (
                                    <tr key={inv.id} className="hover:bg-opacity-50 transition-colors cursor-pointer"
                                        style={{
                                            backgroundColor: 'transparent',
                                            ':hover': { backgroundColor: `${theme.primary}05` }
                                        }}
                                        onMouseOver={(e) => e.currentTarget.style.backgroundColor = `${theme.primary}05`}
                                    ...
                                    >
                                        ...
                                        <td className="px-8 py-6">
                                            <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border`}
                                                style={{
                                                    backgroundColor: inv.status === 'Paid' ? `${theme.success}10` : inv.status === 'Pending' ? `${theme.warning}10` : `${theme.error}10`,
                                                    color: inv.status === 'Paid' ? theme.success : inv.status === 'Pending' ? theme.warning : theme.error,
                                                    borderColor: inv.status === 'Paid' ? `${theme.success}30` : inv.status === 'Pending' ? `${theme.warning}30` : `${theme.error}30`
                                                }}
                                            >
                                                {inv.status}
                                            </span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-right-4 duration-500">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="p-10 rounded-[2.5rem] shadow-premium border" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                            ...
                            <div className="mt-8 pt-8 border-t flex justify-end gap-3" style={{ borderColor: theme.cardBorder }}>
                                <button className="px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest opacity-70 hover:opacity-100 transition-opacity" style={{ color: theme.textPrimary }}>Test Connection</button>
                                <button className="px-8 py-3 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:shadow-xl transition-all" style={{ background: theme.gradients.button }}>Update Credentials</button>
                            </div>
                        </div>

                        <div className="p-8 rounded-[2.5rem] shadow-premium border-l-4" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder, borderLeftColor: theme.secondary }}>
                            <div className="flex gap-4">
                                <FiInfo className="mt-1" size={20} style={{ color: theme.secondary }} />
                                <div>
                    ...
                    <div className="space-y-6">
                        <div className="p-6 rounded-[2.5rem] shadow-premium border" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between text-xs font-bold">
                                    ...
                                    <span style={{ color: theme.warning }}>Test Mode</span>
                                </div>
                                <div className="flex justify-between text-xs font-bold">
                        ...
                        <div className="p-6 rounded-[2.5rem] shadow-premium border-none relative overflow-hidden text-white" style={{ background: theme.gradients.sidebar }}>
                            ...
                            <div className="flex items-center gap-3 relative z-10">
                                <FiShield className="text-white opacity-80" size={24} />
            ...
            <div className="p-10 rounded-[2.5rem] text-white flex justify-between items-center shadow-brand-xl relative overflow-hidden"
                style={{ background: theme.gradients.primary }}
            >
                <div className="absolute top-0 right-0 w-80 h-80 bg-white opacity-10 rounded-full -mr-40 -mt-40 blur-3xl"></div>
                <div className="flex items-center gap-8 relative z-10">
                    ...
                    <div>
                        ...
                        <p className="font-medium opacity-80 max-w-sm">Enable automatic email notifications for clients when payment milestones are reached.</p>
                    </div>
                </div>
                <button className="px-8 py-4 bg-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:scale-105 transition-all relative z-10" style={{ color: theme.primary }}>Configure Workflow</button>
            </div>
                    ...
                    >
                        <div className="p-6 text-white relative flex justify-between items-center" style={{ background: theme.gradients.primary }}>
                            <div>
                                ...
                                <p className="text-white/80 text-[10px] font-bold uppercase tracking-widest mt-1">Create New Payment Request</p>
                            </div>
                            <button onClick={() => setShowInvoiceModal(false)} className="p-2 bg-white/10 hover:bg-white/20 rounded-xl transition-colors text-white">
                                <FiX />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleCreateInvoice} className="p-8 space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest opacity-60" style={{ color: theme.textPrimary }}>Project Name</label>
                                <input type="text" required value={newInvoice.project} onChange={e => setNewInvoice({ ...newInvoice, project: e.target.value })}
                                    ...
                                    placeholder="e.g. The Zenith Residency"
                                />
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest opacity-60" style={{ color: theme.textPrimary }}>Milestone / Type</label>
                                <select value={newInvoice.type} onChange={e => setNewInvoice({ ...newInvoice, type: e.target.value })}
                                    className="w-full p-3 rounded-xl border font-bold text-sm focus:outline-none focus:ring-2"
                                    style={{ borderColor: theme.cardBorder, color: theme.textPrimary, backgroundColor: `${theme.primary}05` }}
                                >
                                    <option>Phase 1: Concept</option>
                                    <option>Phase 2: Schematic</option>
