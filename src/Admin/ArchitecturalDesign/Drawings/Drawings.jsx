            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {drawings.map((doc) => (
                    <div key={doc.id} className="group relative bg-white dark:bg-brand-900/30 p-8 rounded-[2.5rem] shadow-premium border border-brand-50/50 dark:border-brand-800/50 hover:shadow-premium-xl transition-all duration-500 overflow-hidden">
                        <div className="absolute top-0 right-0 p-6">
                            <button className="text-gray-400 hover:text-brand-600 transition-colors"><FiMoreVertical /></button>
                        </div>
                        ...
                        <div className="mt-8 pt-8 border-t border-brand-50 dark:border-brand-800 space-y-4">
                            <div className="space-y-1">
                                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Phase</p>
                                <p className="text-xs font-bold text-gray-700 dark:text-brand-200">{doc.phase}</p>
                            </div>
                            <div className="flex justify-between items-end">
                                <div>
                                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Size / Date</p>
                                    <p className="text-xs font-bold text-gray-500">{doc.size} • {doc.date}</p>
                                </div>
                ...
                <div className="hidden lg:flex border-4 border-dashed border-brand-50 dark:border-brand-800/40 rounded-[2.5rem] items-center justify-center p-12 hover:border-brand-600/40 transition-colors cursor-pointer group">
                    <div className="flex flex-col items-center text-center space-y-4">
                        ...
                        <p className="text-[10px] font-black text-gray-400 dark:text-brand-400 uppercase tracking-widest">Add New Drawing</p>
                    </div>
                </div>
            </div>
        </div>
